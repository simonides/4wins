...
class IntelAI : public I_AI {
private:
    const bool intelligentMeepleChoosing;
    const bool intelligentMeeplePositioning;


    int* buildThinkingMap(const BoardState& board, const Meeple& meepleToSet, const MeepleBag& opponentBag) const;      //Note: the return-value has to be deleted[]
    int getPointsForCombination(const Meeple *meeple[4], const Meeple& meepleToSet, const MeepleBag& opponentBag) const;
...
public:
    IntelAI(bool intelligentMeepleChoosing, bool intelligentMeeplePositioning);
